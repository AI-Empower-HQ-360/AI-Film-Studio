# ============================================================================
# AI Film Studio - Dev Environment Terraform Configuration
# ============================================================================
# Created: 2025-12-27
# Environment: Development
# Description: Complete infrastructure setup for AI Film Studio dev environment
# ============================================================================
...
resource "aws_cloudwatch_log_group" "rds" {
  name              = "/aws/rds/${local.project}-postgres-${local.environment}"
  retention_in_days = 7

  tags = merge(local.common_tags, {
    Name = "${local.project}-rds-logs-${local.environment}"
  })
}
...
resource "aws_cloudwatch_metric_alarm" "rds_storage" {
  alarm_name          = "${local.project}-rds-storage-${local.environment}"
  ...
  tags = merge(local.common_tags, {
    Name = "${local.project}-rds-storage-alarm-${local.environment}"
  })
}
...
resource "aws_cloudwatch_metric_alarm" "ecs_cpu" {
  alarm_name          = "${local.project}-ecs-backend-cpu-${local.environment}"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 2
  metric_name         = "CPUUtilization"
  namespace           = "AWS/ECS"
  ...
  tags = merge(local.common_tags, {
    Name = "${local.project}-ecs-cpu-alarm-${local.environment}"
  })
}

# ECS Service Memory Utilization
resource "aws_cloudwatch_metric_alarm" "ecs_memory" {
  alarm_name          = "${local.project}-ecs-backend-memory-${local.environment}"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 2
  ...
  tags = merge(local.common_tags, {
    Name = "${local.project}-ecs-memory-alarm-${local.environment}"
  })
}
